...
Given("el usuario esta en la pagina de lambdatest", async () => {
...
When("el usuario clickea en el boton de 'Register'", async function () {
  for (const page of pages) {
...
When('el usuario ingresa los datos validos:', async function (DataTable) {
  for (const page of pages) {
...
When("el usuario clickea en el checkbox de 'Privacy Policy'", async function () {
  for (const page of pages) {
...
Then("el usuario deberia ver un mensaje de registro exitoso", async function () {
  for (const page of pages) {
   await page.getByRole("link", {name: success}).click();
  }
});

Then("el usuaria clickea en el logout", async function () {
  for (const page of pages) {
